...
export default function Alerta({ alerta }) {
  ...
  useEffect(() => {
    if (alerta.error) {
      setShow(alerta.error);

      // Configura un temporizador para cerrar la alerta después de 5 segundos
      const timer = setTimeout(() => {
        setShow(false);
      }, 5000);

      // Limpia el temporizador cuando el componente se desmonta o cuando alerta cambia
      return () => {
        clearTimeout(timer);
      };
    }
          ...
          {show && (
            ...
            >
              <div className="flex">
                ...
                >
                  {`${
                    alerta.error ? "Tuvimos un problema." : "Todo salió bien."
                  }`}
                </div>
